...
def test_data_loader(tmp_path):
    ...
    fig_folder = tmp_path / f"{args.folder_path}M"
    os.makedirs(fig_folder, exist_ok=True)
    os.makedirs(fig_folder / "lrs", exist_ok=True)
    os.makedirs(fig_folder / "loss", exist_ok=True)
    ...
    for file_name in FILES:
        ...
        plt.legend()
        plt.savefig(fig_folder / "lrs" / f"{file_id}_lrs.png")
        plt.close()
        plt.plot(step, loss, label=file_id)
        plt.legend()
        plt.savefig(fig_folder / "loss" / f"{file_id}_loss.png")
        plt.close()
